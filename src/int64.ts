...
export default class Int64 {
    ...
    constructor(a1: Buffer | Uint8Array | string | number, a2?: number) {
        if (a1 instanceof Buffer) {
            ...
            this.buffer = new Buffer(a1 as Uint8Array);
            this.offset = a2 || 0;
        } else {
    ...
    setValue(hi: string | number, lo?: number): void {
        ...
        var b = this.buffer,
            o = this.offset;
        for (let i = 7; i >= 0; i--) {
            b[o + i] = (lo as number) & 0xff;
            lo = i == 4 ? (hi as number) : (lo as number) >>> 8;
        }
